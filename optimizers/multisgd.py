...
class MultiSGD(optimizer_v2.OptimizerV2):
  ...
  def _prepare_local(self, var_device, var_dtype, apply_state):
    super(MultiSGD, self)._prepare_local(var_device, var_dtype, apply_state)
    apply_state[(var_device, var_dtype)]["momentum"] = array_ops.identity(
        self._get_hyper("momentum", var_dtype))

  def get_multiplier(self, var):
    return self.lr_multi.get(var.name) or 1.0
  ...
  def _resource_apply_dense(self, grad, var, apply_state=None):
    ...
    multiplier = self.get_multiplier(var)
    ...
    if self._momentum:
      ...
      return training_ops.resource_apply_gradient_descent(
          var.handle, coefficients["lr_t"] * multiplier, grad, use_locking=self._use_locking)

  def _resource_apply_sparse_duplicate_indices(self, grad, var, indices,
                                               **kwargs):
    if self._momentum:
      return super(MultiSGD, self)._resource_apply_sparse_duplicate_indices(
      ...
      return resource_variable_ops.resource_scatter_add(
          var.handle, indices, -grad * coefficients["lr_t"] * multiplier)
  ...
  def get_config(self):
    config = super(MultiSGD, self).get_config()
    config.update({
        "learning_rate": self._serialize_hyperparameter("learning_rate"),
        "lr_multi": self._serialize_hyperparameter("lr_multi"),
